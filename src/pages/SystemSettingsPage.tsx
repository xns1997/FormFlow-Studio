          ...
          <div className="settings-form">
            {section === 'general' && (
              <>
                ...
                <label className="toggle"><input type="checkbox" checked={settings.general.autoOpenLastProject} onChange={(e) => updateSettings((current) => ({ ...current, general: { ...current.general, autoOpenLastProject: e.target.checked } }))} /><span>启动时自动恢复最近项目</span></label>
              </>
            )}
            {section === 'storage' && (
              <>
                ...
                <label className="toggle"><input type="checkbox" checked={settings.storage.preferOfflineSave} onChange={(e) => updateSettings((current) => ({ ...current, storage: { ...current.storage, preferOfflineSave: e.target.checked } }))} /><span>优先离线兼容保存</span></label>
              </>
            )}
            {section === 'editor' && (
              <>
                ...
                <label className="toggle"><input type="checkbox" checked={settings.editor.lineNumbers} onChange={(e) => updateSettings((current) => ({ ...current, editor: { ...current.editor, lineNumbers: e.target.checked } }))} /><span>显示行号</span></label>
                <label className="toggle"><input type="checkbox" checked={settings.editor.suggestionDocs} onChange={(e) => updateSettings((current) => ({ ...current, editor: { ...current.editor, suggestionDocs: e.target.checked } }))} /><span>显示补全文档</span></label>
              </>
            )}
            {section === 'experiments' && (
              <>
                <label className="toggle"><input type="checkbox" checked={settings.experiments.enableNewRouter} onChange={(e) => updateSettings((current) => ({ ...current, experiments: { ...current.experiments, enableNewRouter: e.target.checked } }))} /><span>启用新路由分层</span></label>
                <label className="toggle"><input type="checkbox" checked={settings.experiments.enablePreviewDocs} onChange={(e) => updateSettings((current) => ({ ...current, experiments: { ...current.experiments, enablePreviewDocs: e.target.checked } }))} /><span>启用文档预览入口</span></label>
              </>
            )}
          </div>
        </div>
