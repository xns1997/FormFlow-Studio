...
import { getTemplatesByCategory, type BehaviorTemplate } from '../services/behaviorTemplates';
import Modal, { ModalHeader } from '../components/Modal';
import BehaviorTestPanel from '../components/BehaviorTestPanel';
import RuleBuilder from '../components/RuleBuilder';
import { downloadBehaviors, importBehaviors, readFileAsText } from '../services/behaviorIO';
import {
      ...
      <div className="page-sidebar">
        <div className="page-section-header">
          ...
          <div style={{ display: 'flex', gap: 4 }}>
            ...
            <button onClick={() => importRef.current?.click()} style={{ padding: '2px 6px', fontSize: 10, border: '1px solid var(--line)', borderRadius: 4, background: 'var(--panel)' }} title="导入">↑</button>
            <button onClick={handleExport} disabled={scripts.length === 0} style={{ padding: '2px 6px', fontSize: 10, border: '1px solid var(--line)', borderRadius: 4, background: 'var(--panel)', opacity: scripts.length === 0 ? 0.5 : 1 }} title="导出">↓</button>
            <button onClick={() => setShowTemplates(true)} style={{ padding: '2px 8px', fontSize: 11, border: '1px solid var(--line)', borderRadius: 4, background: 'var(--panel)' }}>模板</button>
              ...
              <div style={{ display: 'flex', border: '1px solid var(--line)', borderRadius: 6, overflow: 'hidden' }}>
                <button
                  ...
                  style={{
                    ...
                    background: editorMode === 'visual' ? 'var(--accent-soft)' : 'transparent',
                    color: editorMode === 'visual' ? 'var(--accent)' : 'var(--muted)',
                ...
                >
                  ...
                  style={{
                    padding: '3px 8px', fontSize: 10, border: 'none',
                    background: editorMode === 'code' ? 'var(--accent-soft)' : 'transparent',
                    color: editorMode === 'code' ? 'var(--accent)' : 'var(--muted)',
      ...
      <div className="page-inspector">
        <div className="page-section-header" style={{ display: 'flex', gap: 0, padding: 0 }}>
          <button
            ...
            style={{
              flex: 1, padding: '8px 12px', fontSize: 11, fontWeight: rightPanelTab === 'api' ? 600 : 400,
              border: 'none', borderBottom: rightPanelTab === 'api' ? '2px solid var(--accent)' : '2px solid transparent',
              background: 'transparent', color: rightPanelTab === 'api' ? 'var(--accent)' : 'var(--muted)',
              cursor: 'pointer',
            }}
          >
            Reference
          ...
          <button
            ...
            style={{
              flex: 1, padding: '8px 12px', fontSize: 11, fontWeight: rightPanelTab === 'test' ? 600 : 400,
              border: 'none', borderBottom: rightPanelTab === 'test' ? '2px solid var(--accent)' : '2px solid transparent',
              background: 'transparent', color: rightPanelTab === 'test' ? 'var(--accent)' : 'var(--muted)',
              cursor: 'pointer',
            }}
          >
            测试
        ...
        <div style={{ padding: '0 0 8px' }}>
          {Object.entries(getTemplatesByCategory()).map(([category, templates]) => (
            <div key={category} style={{ marginBottom: 12 }}>
              <div style={{ fontSize: 11, fontWeight: 700, color: 'var(--muted)', textTransform: 'uppercase', padding: '8px 16px 4px', letterSpacing: '0.5px' }}>{category}</div>
              {templates.map((tpl) => (
                <div
                  key={tpl.id}
                ...
                >
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                    ...
                    <span style={{ fontSize: 10, color: 'var(--muted)', fontFamily: 'monospace', padding: '1px 5px', background: 'var(--panel-soft)', borderRadius: 3 }}>{tpl.event}</span>
                  </div>
                  <span style={{ fontSize: 11, color: 'var(--muted)' }}>{tpl.description}</span>
                  {tpl.fields && tpl.fields.length > 0 && (
                    <div style={{ display: 'flex', gap: 4, marginTop: 2 }}>
                      {tpl.fields.map((f) => (
                        <span key={f} style={{ fontSize: 9, padding: '1px 5px', background: 'var(--accent-soft)', color: 'var(--accent)', borderRadius: 3 }}>{f}</span>
                      ))}
                    </div>
                  )}
                </div>
