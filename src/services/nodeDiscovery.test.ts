...
test('all 131 registered nodes build searchable documents and map to eight discovery groups', async () => {
  ...
  const index = buildNodeSearchIndex(specs);
  assert.equal(index.length, 131);
  assert.equal(new Set(index.map((document) => document.spec.id)).size, 131);
