...
function fail(args: Record<string, any>, code: string, message: string, path: string | undefined, expectedShape: unknown, suggestedArguments?: Record<string, unknown>): ToolPreflightError {
  return { code, message, path, expectedShape, receivedShape: shape(args), suggestedArguments };
}
function validateScope(args: Record<string, any>): ToolPreflightError | undefined {
...
/**
 * Behavior-domain validation hook for the unified argument pipeline: frozen
 * FormFlow v2 behavior shapes, exact scopes, and load-bearing action rules.
 */
export const behaviorToolDomainHooks: ToolDomainHooks = {
  validate(name, args) {
    if (!['behavior.list', 'behavior.upsert', 'behavior.delete'].includes(name)) return undefined;
    const scopeFailure = validateScope(args); if (scopeFailure) return scopeFailure;
    if (name === 'behavior.list') return undefined;
    if (name === 'behavior.delete') return args.id ? undefined : fail(args, 'BEHAVIOR_ID_REQUIRED', '删除行为必须提供稳定 id', 'id', { id: 'behavior-id' });
...
/** Wrapper kept for tests and callers that preflight behavior arguments directly. */
export function compileBehaviorToolArguments(name: string, original: Record<string, any>): ToolPipelineResult {
  return compileToolArgumentsPipeline(name, original, undefined, { validate: behaviorToolDomainHooks.validate });
}
