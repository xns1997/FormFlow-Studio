...
- 渐进式属性面板（搜索、常用/高级、配置摘要、恢复默认、复制粘贴）
...
当前质量基线：TypeScript 类型检查、生产构建、198 项 `.test.ts` 单元测试与 7 项 Playwright E2E 通过。`pnpm test` 使用跨平台收集脚本递归发现测试文件，不依赖 shell glob 行为。
