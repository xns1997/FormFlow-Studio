...
本仓库当前发布版本为 **1.2.0**。本版本完成共享表单控件 Schema、设计预览、运行态表单、模板工作台和七领域 MCP 的交互收口，并为所有已纳入范围的 TODO 提供单测、服务端和 Playwright 证据；详细变更见 [`CHANGELOG.md`](./CHANGELOG.md)。
...
- **流程绑定 V2**：事件配置内直接选择流程并编辑输入/输出映射；稳定 I/O 字段 ID 支持字段改名不断链，旧 `parameterMap` 和同名输出回写只在用户应用配置时迁移。
- **结果可预期**：映射编辑器显式展示完成度、自动匹配、未映射、失效、覆盖和类型风险；输出先整体计算与校验，再原子回写，避免部分字段已更新而其他字段失败。
- **统一数据变更入口**：REST、MCP 和表格批量写回共用 revision、幂等、访问控制和破坏性确认语义，稳定 rowKey 可跨分页追踪修改。
- **共享领域核心**：行为 DSL、属性表达式与依赖、字段推断、表单脚手架和事件契约集中到 `shared/formflow-core`，浏览器适配留在 UI 边界。
- **可部署构建**：前端、服务端和测试使用独立 TypeScript 边界；生产构建同时生成 UI 与 Server 产物，Docker 和发布流水线执行相同门禁。
...
- 流程触发器配置（控件事件 → 运行流程），在单层编辑器中完成目标流程、输入来源、输出回写与转换配置
- 流程绑定风险总览同时使用文字、图标和无障碍标签；支持迁移预览、高级 V2 JSON、输入/输出试算，以及携带草稿返回会话的流程节点定位
...
当前质量基线由 TypeScript 类型检查、生产构建、`.test.ts` 单元/集成测试与 Playwright E2E 组成。`pnpm verify` 是提交前统一门禁；`pnpm test` 使用跨平台收集脚本递归发现测试文件，不依赖 shell glob 行为。
