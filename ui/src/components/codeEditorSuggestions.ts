...
import {
  FORM_EVENT_CONTRACT,
  renderFormEventContractAliases,
...
export function createEventContextSuggestions(options: {
  ...
  const contractSuggestions = FORM_EVENT_CONTRACT
    .filter((member) => !member.internal)
    .map<CodeEditorSuggestion>((member, index) => ({
      label: `ctx.${member.name}`,
      insertText: member.kind === 'method' ? `ctx.${member.name}` : undefined,
      kind: member.kind === 'method' ? 'Function' : 'Field',
      detail: member.description,
      sortText: `000${index.toString().padStart(3, '0')}`,
      scope: 'ctx-member',
    }));
  return [
    ...contractSuggestions,
    ...
    { label: 'ctx.flow', insertText: "await ctx.flow('流程 ID').run({ formData: ctx.form.values() })", kind: 'Snippet', detail: '链式运行流程；输出按事件的显式 V2 绑定统一处理', sortText: '0454', scope: 'ctx-member' },
    ...
    ...(options.components || []).flatMap<CodeEditorSuggestion>((component, index) => {
      ...
      const label = String(component.props?.label || component.props?.name || canonical);
      const suffix = index.toString().padStart(3, '0');
      return [
        { label: id, insertText: id, kind: 'Reference', detail: `控件：${label} · ${component.type}`, sortText: `16${suffix}`, scope: 'component-name' },
        { label: canonical, insertText: canonical, kind: 'Reference', detail: `控件句柄：${label} · ${component.type}`, sortText: `17${suffix}`, scope: 'ctx-controls-member' },
        { label: id, insertText: id, kind: 'Reference', detail: `控件 ID 句柄：${label} · ${component.type}`, sortText: `172${suffix}`, scope: 'ctx-controls-member' },
        { label: `ctx.controls.${canonical}`, kind: 'Field', detail: `控件句柄：${label} · ${component.type}`, sortText: `171${suffix}`, scope: 'ctx-member' },
        { label: `ctx.controls.${id}`, kind: 'Field', detail: `控件 ID 句柄：${label} · ${component.type}`, sortText: `173${suffix}`, scope: 'ctx-member' },
      ];
    }),
...
export interface EventContextExtraLibOptions {
  filePath: string;
  fields?: Array<string | EventFieldDescriptor>;
  currentField?: string;
  eventName?: string;
...
function createHandwrittenEventContextExtraLib(options: EventContextExtraLibOptions): CodeEditorExtraLib {
  const descriptors = (options.fields || []).map((field) => typeof field === 'string' ? { name: field } : field).filter((field) => field.name);
  const fields = [...new Map(descriptors.map((field) => [field.name, field])).values()];
  const currentField = fields.find((field) => field.name === options.currentField);
...
export function createEventContextExtraLib(options: EventContextExtraLibOptions): CodeEditorExtraLib {
  ...
  let specialized = library.content.replace(
    'interface FormEventContext {',
    'interface FormEventContext extends FormEventCanonicalContract {',
  );
  const contextStart = specialized.indexOf('interface FormEventContext extends FormEventCanonicalContract {');
  const contextEnd = specialized.indexOf('type FormEventHandler', contextStart);
  const prefix = specialized.slice(0, contextStart);
  let contextDeclaration = specialized.slice(contextStart, contextEnd);
  let suffix = specialized.slice(contextEnd);
  // Canonical generic declarations are generated once. The specialized
  // declaration below retains only field-aware overloads and refinements.
  for (const member of FORM_EVENT_CONTRACT) {
    const declaration = member.kind === 'method'
      ? `  ${member.signature};`
      : `  ${member.name}: ${member.type};`;
    contextDeclaration = contextDeclaration.replace(`${declaration}\n`, '');
    if (member.topLevelAlias) {
      const alias = member.kind === 'method'
        ? `declare function ${member.signature};`
        : `declare const ${member.name}: ${member.type};`;
      suffix = suffix.replace(`${alias}\n`, '');
    }
  }
  specialized = `${prefix}${contextDeclaration}${suffix}`;
  return {
    ...library,
    content: `${renderFormEventContractInterface()}\n${renderFormEventContractAliases()}\n${specialized}`,
  };
}
...
interface FormFlowFormChain {
  values(): EventFieldMap & Record<string, unknown>;
  require(fields: string[]): FormFlowRequireChain;
}
interface FormFlowTableChain {
...
interface FormFlowChain {
  run(parameters?: Record<string, unknown>, options?: { targetNodeId?: string }): FormFlowRunChain;
}
interface FormEventContext {
