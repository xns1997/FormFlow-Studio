...
import { registerBehaviorDslLanguageServices } from './behaviorDslLanguageServices';
...
export default function BehaviorDslEditor({ projectId, value, onChange, fields, components, tables, workflows, formId, formName, onApply, onProposalApplied }: Props) {
  ...
  return <div className="behavior-dsl-editor">
    ...
    <div className="behavior-dsl-code"><div className="behavior-dsl-templates">{BEHAVIOR_DSL_TEMPLATES.map((template) => <button key={template.label} type="button" className="ui-btn ui-btn-xs" onClick={() => appendRule(template.value)}>{template.label}</button>)}</div><CodeEditor value={value} onChange={onChange} language={BEHAVIOR_DSL_LANGUAGE_ID} path={createBehaviorRuleModelPath(formId)} theme="light" lineNumbers suggestions={suggestions} suggestionContextResolver={resolveBehaviorDslCompletionContext} autoSuggestPolicy="contextual" suggestionTriggerCharacters={['$', '@', ' ', '>', '=', '(', ',', ';']} beforeMount={registerBehaviorDslLanguage} onMount={handleMount} providers={[(monaco, editor) => registerBehaviorDslLanguageServices(monaco, editor, { fields, components, tables, workflows })]} options={{ minimap: { enabled: false }, folding: true, inlineSuggest: { enabled: true }, fontSize: 13, lineHeight: 22, wordBasedSuggestions: 'off' }} fullscreen /><small>按 Ctrl+Space 查看补全；悬停、参数提示、快速修复、折叠、格式化与内联补全已启用。动作统一使用函数形式，以分号分隔；字段使用 $，控件使用 @。</small></div>
