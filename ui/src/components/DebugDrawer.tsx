...
export default function DebugDrawer({
  ...
  const [source, setSource] = useState<'all' | DebugEntrySource>('all');
    ...
    <div style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 12px', borderBottom: open ? '1px solid rgba(148,163,184,0.18)' : 'none' }}>
        <div>
          ...
          <div style={{ fontSize: 10, color: '#64748b' }}>{mergedEntries.length} 条</div>
        </div>
              ...
              return (
                ...
                >
                  <div style={{ display: 'flex', justifyContent: 'space-between', gap: 8, marginBottom: 4 }}>
                    <div style={{ fontSize: 11, fontWeight: 700, color: tone }}>{entry.title || entry.source}</div>
                    <div style={{ fontSize: 10, color: '#64748b' }}>{new Date(entry.timestamp).toLocaleTimeString()}</div>
                  </div>
                  <div style={{ fontSize: 11, color: '#0f172a', wordBreak: 'break-word' }}>{entry.message}</div>
