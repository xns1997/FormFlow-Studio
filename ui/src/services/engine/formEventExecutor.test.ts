...
test('ctx batch helpers and toggle helpers update runtime values and state consistently', async () => {
  const writes: Array<[string, unknown]> = [];
  const visibleCalls: Array<[string, boolean]> = [];
  const disabledCalls: Array<[string, boolean]> = [];
  const requiredCalls: Array<[string, boolean]> = [];
  const peer: ComponentNode = {
    id: 'peer-1',
    type: 'input',
    name: 'approvalComment',
    label: '审批意见',
    props: {},
    layout: { row: 0, col: 1, colSpan: 1, rowSpan: 1 },
    ports: [],
    events: [],
  };
  const result = await executeFormControlEvent(context, {
    workflows: [],
    components: [component, peer],
    setValue: (field, value) => { writes.push([field, value]); },
    setVisible: (componentId, visible) => { visibleCalls.push([componentId, visible]); },
    setDisabled: (componentId, disabled) => { disabledCalls.push([componentId, disabled]); },
    setRequired: (field, required) => { requiredCalls.push([field, required]); },
    code: `async (ctx) => {
      const before = ctx.getValues(['customerName', 'approvalComment']);
      await ctx.setValues({ customerName: '批量客户', approvalComment: '已填写' });
      await ctx.clearValue('approvalComment');
      await ctx.toggleVisible('peer-1');
      await ctx.toggleDisabled('peer-1');
      await ctx.toggleRequired('customerName');
      await ctx.setFieldState('customerName', { value: '最终客户', required: false });
      await ctx.setFieldState('peer-1', { visible: true, disabled: false });
      return {
        before,
        after: ctx.getValues(['customerName', 'approvalComment']),
      };
    }`,
  });
  assert.equal(result.error, undefined);
  assert.deepEqual(result.callbackResult, {
    before: { customerName: '新客户', approvalComment: undefined },
...
test('ctx focus scroll and tab helpers resolve runtime targets', async () => {
  const writes: Array<[string, unknown]> = [];
  const tabsComponent: ComponentNode = {
    id: 'tabs-1',
    type: 'tabs',
    name: 'mainTabs',
    label: '主标签页',
    props: { tabs: ['基本信息', '审批信息'] },
    layout: { row: 0, col: 0, colSpan: 2, rowSpan: 1 },
    ports: [],
    events: [],
  };
  const nestedField: ComponentNode = {
    id: 'field-2',
    type: 'input',
    name: 'approvalComment',
    label: '审批意见',
    props: { parentId: 'tabs-1' },
    layout: { row: 1, col: 0, colSpan: 1, rowSpan: 1 },
    ports: [],
    events: [],
  };
  ...
  const targetNode = {
    focus: () => hostEvents.push('focus'),
    scrollIntoView: () => hostEvents.push('scroll'),
    querySelector: () => null,
  };
  const hostRoot = {
  ...
  const result = await executeFormControlEvent({
    ...
    value: undefined,
    values: { approvalComment: '', mainTabs: 0 },
    originalValues: { approvalComment: '', mainTabs: 0 },
    component: nestedField,
  }, {
    workflows: [],
    components: [tabsComponent, nestedField],
    hostRoot,
    setValue: (field, value) => { writes.push([field, value]); },
    code: `async (ctx) => {
      await ctx.scrollToField('approvalComment');
      await ctx.focusField('approvalComment');
      await ctx.switchTab('审批信息');
      await ctx.openTab(0);
      return ctx.getValue('mainTabs');
    }`,
  });
  assert.equal(result.error, undefined);
