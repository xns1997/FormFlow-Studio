...
import { FORM_WINDOW_CONTROL } from './formWindowModel';
...
export function PropertyPanel({ component, components = [], onUpdate, onUpdateGeometry, onRemove, onClose, onEditWorkflowContract, onFinishWorkflowContractSession }: Props) {
  ...
  useEffect(() => { setQuery(''); setActiveTask(undefined); setShowAdvanced(false); }, [component?.id]);
  ...
  const isFormWindow = component.type === FORM_WINDOW_CONTROL.type;
  const control = isFormWindow ? FORM_WINDOW_CONTROL : getControl(component.type);
  ...
  return <div className="designer-properties-shell"><FormAntdProvider><div className="designer-properties">
    <PropertyPanelHeader type={component.type} label={control.label} icon={control.icon} query={query} section={section} onQuery={setQuery} onSection={setSection} onRemove={isFormWindow ? undefined : () => onRemove?.(component.id)} onClose={onClose} />
