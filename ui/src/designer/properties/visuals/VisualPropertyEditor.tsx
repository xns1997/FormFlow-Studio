...
import {
  ArrayRowsVisual,
  DataBindingVisual,
  DateBusinessDayConfigVisual,
  DateConstraintConfigVisual,
  DateDefaultConfigVisual,
  KeyValueVisual,
  OptionAdvancedVisual,
  OptionContentVisual,
  OptionSourceVisual,
  OptionsVisual,
  StringListVisual,
} from './CollectionPropertyEditors';
import { DisplayConditionsVisual, ExpressionVisual, RegexVisual, ValidationRulesVisual } from './LogicPropertyEditors';
...
export function VisualPropertyEditor({ kind, draft, context, setDraft, setValid }: { kind: string; draft: unknown; context: PropertyEditorContext; setDraft: (value: unknown) => void; setValid: (value: boolean) => void }) {
  ...
  if (kind === 'option-content') return <OptionContentVisual value={draft} context={context} onChange={setDraft as any} onValidity={setValid} />;
  if (kind === 'option-advanced') return <OptionAdvancedVisual value={draft} onChange={setDraft as any} onValidity={setValid} />;
  if (kind === 'date-default-config') return <DateDefaultConfigVisual value={draft} context={context} onChange={setDraft as any} onValidity={setValid} />;
  if (kind === 'date-constraint-config') return <DateConstraintConfigVisual value={draft} context={context} onChange={setDraft as any} onValidity={setValid} />;
  if (kind === 'date-business-day-config') return <DateBusinessDayConfigVisual value={draft} onChange={setDraft as any} onValidity={setValid} />;
  ...
  if (kind === 'display-conditions') return <DisplayConditionsVisual value={draft} context={context} onChange={setDraft as any} onValidity={setValid} />;
  ...
  if (['number-range', 'date-range', 'selection-range', 'spacing', 'typography', 'border', 'shadow', 'upload-constraints'].includes(kind)) return <CompositeVisual kind={kind} value={draft} context={context} onChange={setDraft} onValidity={setValid} />;
