...
export function FlowTriggerEditor({
  ...
  const [targetFilter, setTargetFilter] = useState<'all' | 'variable' | 'nodePort' | 'mapped' | 'unmapped'>('all');
  ...
  const autoMapVariableRows = useCallback(() => {
    commitRows(draftRows.map((row) => {
      if (row.targetType !== 'variable') return row;
      const matchedField = findMatchingField(row.targetKey, fields);
      if (!matchedField) return row;
      return { ...row, enabled: true, valueMode: 'fieldValue', value: matchedField };
    }));
  }, [commitRows, draftRows, fields]);
  ...
  const filteredRows = draftRows.filter((row) => {
    ...
    if (targetFilter === 'variable') return row.targetType === 'variable';
  ...
  const visibleVariableRows = variableRows.filter((row) => visibleRowIds.has(row.id));
  ...
  const variableValueModeOptions = [
  ...
  const renderMappingRow = (row: FlowParameterDraftRow) => {
    ...
    if (row.targetType === 'variable') {
      return (
        <div key={row.id} className={`flow-mapping-table-row ${isRowConfigured(row) ? 'configured' : 'pending'}`}>
          <div className="flow-mapping-row-main flow-mapping-grid-variable">
            ...
            <AntdSelectInput
              value={row.valueMode}
              options={variableValueModeOptions}
            ...
            <div className="flow-mapping-value-cell">
              {valueInput || <div className="flow-mapping-auto-note">使用系统事件值，无需补充参数</div>}
            </div>
            ...
            <div className="prop-flow-trigger-summary">
              ...
              {workflow && variableNames.length > 0 && (
                <div className="prop-flow-trigger-vars">
                  <span>流程变量</span>
                  {variableNames.slice(0, 6).map((name) => <code key={name}>{name}</code>)}
                  {variableNames.length > 6 && <code>+{variableNames.length - 6}</code>}
                </div>
              )}
          ...
          {mode === 'ui' ? (
            <>
              <div className="flow-mapping-toolbar">
                <AntdTextInput value={searchText} placeholder="搜索变量、端口或路径" onChange={setSearchText} />
                <AntdSelectInput
                  value={targetFilter}
                  options={[
                    { label: '全部映射', value: 'all' },
                    { label: '变量参数', value: 'variable' },
                    { label: '节点端口', value: 'nodePort' },
                ...
                />
                <div className="flow-mapping-inline-actions">
                  {showVariableGroup && <button type="button" onClick={autoMapVariableRows}>变量同名自动映射</button>}
                  {showVariableGroup && <button type="button" onClick={() => addRow('variable')}>添加变量</button>}
              ...
              {showVariableGroup && (
                <section className="flow-mapping-section">
                  <div className="flow-mapping-section-head">
                    <strong>变量参数</strong>
                    <span>{visibleVariableRows.length}/{variableRows.length} 行</span>
                  </div>
                  {visibleVariableRows.length === 0 ? (
                    <div className="prop-flow-trigger-empty">当前筛选结果里没有变量映射</div>
                  ) : visibleVariableRows.map(renderMappingRow)}
                </section>
              )}
