...
import { buildSystemSettingsPath } from '../../services/io/routes';
...
const systemSettingsTabs: Array<{ section: SystemSettingsSection; label: string }> = [
  { section: 'general', label: '常规' },
  { section: 'storage', label: '存储' },
  { section: 'editor', label: '编辑器' },
  { section: 'experiments', label: '实验功能' },
];
    ...
    <div className="page-layout">
      <div className="page-main" style={{ borderRight: 'none' }}>
        ...
        <div className="page-section-body">
          ...
          <div className="system-settings-tabs" role="tablist" aria-label="系统设置标签">
            {systemSettingsTabs.map((item) => (
              <Link
                key={item.section}
                to={buildSystemSettingsPath(item.section)}
                className={`system-settings-tab ${section === item.section ? 'active' : ''}`}
              >
                {item.label}
              </Link>
            ))}
