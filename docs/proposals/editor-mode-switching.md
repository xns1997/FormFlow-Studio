...
- P0 命名简化已落地：工作区标签为「数据 / 表单 / 规则 / 流程 / 测试 / 设置」（`ProjectWorkspaceTabs.tsx`）。
- P1 快捷键已落地：`Cmd/Ctrl+1/2/3` 切换表单/规则/流程，`Cmd/Ctrl+,` 进入设置（`UnifiedEditorPage.tsx`）。
- 上下文保留已落地：design/behavior 模式保持挂载，面板选中项与画布状态在模式切换时保留（`UnifiedEditorPage.tsx`）。
- 字段引用面板已落地：规则模式与流程画布左侧均提供「数据字段」页签（`UnifiedEditorPage.tsx`、`CanvasPage.tsx`）。
- 关联式导航与规则编辑器自动补全仍在演进中；若后续实现，请在此处更新状态与涉及文件。
